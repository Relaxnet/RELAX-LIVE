...
const firebaseConfig = {
  apiKey: "AIzaSyAxoM2NXN8hOYFf4K7m7TxAnI8vZN1een0",
  authDomain: "relaxlive-56cdf.firebaseapp.com",
  projectId: "relaxlive-56cdf",
  storageBucket: "relaxlive-56cdf.firebasestorage.app",
  messagingSenderId: "502000959192",
  appId: "1:502000959192:web:f2116c2e4a293d0e2861ad",
  measurementId: "G-W4WVX8M7WQ"
};
...
// وظيفة للحصول على Serial Number مع التحقق من MikroTik
async function getMikrotikSerial() {
  let cachedSerial = localStorage.getItem('mikrotik_serial');
  const urlParams = new URLSearchParams(window.location.search);
  const urlSerial = urlParams.get('serial');

  console.log("جاري التحقق من السريال - URL:", urlSerial, "Cache:", cachedSerial);

  if (urlSerial && urlSerial.trim().length > 0) {
    cachedSerial = urlSerial.trim();
    localStorage.setItem('mikrotik_serial', cachedSerial);
    console.log("تم استعادة السريال من URL:", cachedSerial);
    return cachedSerial;
  }

  try {
    console.log("جاري محاولة جلب /aya/live/firebase.json...");
    const response = await fetch('/././firebase.json', { method: 'GET', mode: 'same-origin' });
    console.log("استجابة الطلب:", response.status, response.statusText);
    if (response.ok) {
      const data = await response.json();
      console.log("محتوى firebase.json:", data);

      if (data.name) {
        const serialFromJson = data.name.split('/').pop().trim();
        if (serialFromJson && serialFromJson.length > 0) {
          cachedSerial = serialFromJson;
          localStorage.setItem('mikrotik_serial', cachedSerial);
          console.log("تم استخراج السريال من firebase.json:", cachedSerial);
          return cachedSerial;
        } else {
          console.warn("السريال غير موجود في حقل name");
        }
      } else {
        console.warn("حقل name غير موجود في firebase.json");
      }
    } else {
      console.warn("فشل الطلب، رمز الحالة:", response.status);
    }
  } catch (error) {
    console.error("خطأ أثناء جلب firebase.json:", error.message);
  }

  if (cachedSerial && cachedSerial.trim().length > 0) {
    console.log("استعادة من localStorage:", cachedSerial);
    return cachedSerial;
  }

  let serialInput;
  do {
    serialInput = prompt('أدخل Serial Number (مثل ABC12345):');
    if (serialInput === null) {
      alert("يجب إدخال Serial Number.");
      continue;
    }
    if (!serialInput || serialInput.trim().length === 0) {
      alert("السريال لا يمكن أن يكون فارغًا.");
    }
  } while (!serialInput || serialInput.trim().length === 0);

  cachedSerial = serialInput.trim();
  localStorage.setItem('mikrotik_serial', cachedSerial);
  console.log("تم حفظ السريال يدويًا:", cachedSerial);
  return cachedSerial;
}

// وظيفة اشتراك المالك
async function subscribeOwner(serial, phone, plan, password) {
  try {
    console.log("بدء عملية الاشتراك - Serial:", serial, "Phone:", phone, "Plan:", plan, "Password:", password);

    if (!phone || !phone.match(/^\+\d{10,14}$/)) {
      throw new Error("رقم الهاتف غير صالح. يرجى استخدام تنسيق مثل +967773035071.");
    }
    ...
    await setDoc(doc(db, "networks", serial), {
      ownerPhone: phone,
      ownerUid: null,
    ...
    console.log("تم حفظ بيانات الاشتراك في Firestore بنجاح.");

    localStorage.setItem('mikrotik_serial', serial);
    alert("تم إرسال طلب الاشتراك بنجاح! في انتظار الموافقة من المسؤول.");
    document.getElementById('subscription-modal').style.display = 'none';
    showError('طلب الاشتراك قيد المراجعة.');
  } catch (error) {
    console.error("خطأ في الاشتراك:", error.message, error);
    alert(`حدث خطأ: ${error.message}. تأكد من البيانات المدخلة والاتصال بالإنترنت.`);
  }
}

// وظيفة تجديد الاشتراك
async function renewSubscription(serial, phone, plan) {
  try {
    const netDoc = await getDoc(doc(db, "networks", serial));
    if (netDoc.exists()) {
      const startDate = new Date();
      let endDate = new Date(startDate);
      let price;
      ...
      alert("تم إرسال طلب التجديد بنجاح! في انتظار الموافقة.");
      document.getElementById('subscription-modal').style.display = 'none';
      showError('طلب التجديد قيد المراجعة.');
    } else {
      alert("لا يوجد اشتراك سابق لهذا الـ Serial.");
    }
  } catch (error) {
...
async function checkSubscription() {
  const serial = await getMikrotikSerial();
  if (!serial) {
    alert("لا يمكن الوصول بدون Serial Number. اتصل بالمالك.");
    const videoPlayer = document.getElementById('video-player');
    if (videoPlayer) videoPlayer.style.display = 'none';
    showError('يرجى إدخال Serial Number صالح.');
    hideLoading();
    return false;
  }
  ...
  if (!subscriptionModal) {
    console.error("خطأ: نافذة الاشتراك 'subscription-modal' غير موجودة في DOM.");
    alert("خطأ في تحميل نموذج الاشتراك. يرجى إعادة تحميل الصفحة.");
    if (loadingOverlay) loadingOverlay.style.display = 'none';
    return false;
  }

  console.log("جاري التحقق من الاشتراك في Firestore للسريال:", serial);
  try {
    const netDoc = await getDoc(doc(db, "networks", serial));
    if (netDoc.exists()) {
      const netData = netDoc.data();
      const endDate = new Date(netData.endDate);
      const currentDate = new Date();

      if (netData.status === 'approved' && netData.active && currentDate < endDate) {
        console.log("اشتراك الشبكة صالح، مرحبا!");
        if (subscriptionModal) subscriptionModal.style.display = 'none';
        if (loadingOverlay) loadingOverlay.style.display = 'none';
        return true;
      } else if (netData.status === 'pending') {
        alert("طلب الاشتراك قيد المراجعة. يرجى الانتظار.");
        if (subscriptionModal) subscriptionModal.style.display = 'block';
        if (videoPlayer) videoPlayer.style.display = 'none';
        if (loadingOverlay) loadingOverlay.style.display = 'none';
        showError('طلب الاشتراك قيد المراجعة.');
        return false;
      } else if (netData.status === 'rejected') {
        alert("تم رفض طلب الاشتراك. يرجى التواصل مع المسؤول.");
        if (subscriptionModal) subscriptionModal.style.display = 'block';
        if (videoPlayer) videoPlayer.style.display = 'none';
        if (loadingOverlay) loadingOverlay.style.display = 'none';
        showError('طلب الاشتراك مرفوض.');
        return false;
      } else {
        alert("انتهى اشتراك الشبكة! يرجى تجديد الاشتراك.");
        if (subscriptionModal) subscriptionModal.style.display = 'block';
        if (videoPlayer) videoPlayer.style.display = 'none';
        if (loadingOverlay) loadingOverlay.style.display = 'none';
        showError('يرجى تجديد الاشتراك.');
        return false;
      }
    } else {
      alert("لا يوجد اشتراك مسجل لهذا الـ Serial. يرجى تسجيل اشتراك جديد.");
      setTimeout(() => {
        if (subscriptionModal) {
          ...
          if (videoPlayer) videoPlayer.style.display = 'none';
          if (loadingOverlay) loadingOverlay.style.display = 'none';
          showError('يرجى تسجيل اشتراك جديد.');
        } else {
          console.error("خطأ: نافذة الاشتراك غير موجودة حتى بعد التأخير.");
          if (loadingOverlay) loadingOverlay.style.display = 'none';
          alert("خطأ في تحميل نموذج الاشتراك. يرجى إعادة تحميل الصفحة.");
        }
      }, 100);
    ...
    console.error("خطأ أثناء التحقق من Firestore:", error.message);
    alert("حدث خطأ أثناء التحقق من الاشتراك. يرجى التأكد من الاتصال بالإنترنت أو التواصل مع الدعم.");
    if (loadingOverlay) loadingOverlay.style.display = 'none';
    return false;
  }
}
  ...
  try {
    ...
    if (enableKeyVerification) {
      ...
      try {
        ...
        console.warn('تحذير: فشل في جلب مفتاح التحقق، سيتم الاستمرار بدون تحقق');
      }
      ...
      if (key) {
        ...
        if (!isValid) {
          console.info('تحذير: المفتاح غير صالح، سيتم الاستمرار في تحميل البث');
        }
      }
    }
    ...
    if (isServerUnavailable) {
      showError('المنصة متاحة في أوقات المباريات فقط');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (subscribeBtn) {
    subscribeBtn.addEventListener('click', async () => {
      ...
      if (serial && phone && plan && password) {
        await subscribeOwner(serial, phone, plan, password);
      } else {
  ...
  if (renewBtn) {
    renewBtn.addEventListener('click', async () => {
      ...
      const phone = document.getElementById('phone-number').value;
      const plan = document.getElementById('subscription-plan').value;
      if (serial && phone && plan) {
        await renewSubscription(serial, phone, plan);
      } else {
...
// تصدير الوظائف
export { checkSubscription, initApp, toggleSettings, toggleMatchesModal, playStream, cleanupPlayer, switchQuality, toggleChannel, playChannelByName, showNotes, getEstimatedConnectionSpeed };
